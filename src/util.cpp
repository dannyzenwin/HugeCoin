...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Huge
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Huge
    // Mac: ~/Library/Application Support/Huge
    // Unix: ~/.Huge
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Huge";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Huge.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "huged.pid"));
