...
namespace {
    class CHugeAddressVisitor : public boost::static_visitor<bool> {
    private:
        CHugeAddress *addr;
    public:
        CHugeAddressVisitor(CHugeAddress *addrIn) : addr(addrIn) { }
...
bool CHugeAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CHugeAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CHugeAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CHugeAddressVisitor(this), dest);
}

bool CHugeAddress::IsValid() const {
...
CTxDestination CHugeAddress::Get() const {
    if (!IsValid())
...
bool CHugeAddress::GetKeyID(CKeyID &keyID) const {
...
bool CHugeAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CHugeSecret::SetKey(const CKey& vchSecret) {
...
CKey CHugeSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CHugeSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CHugeSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CHugeSecret::SetString(const std::string& strSecret) {
