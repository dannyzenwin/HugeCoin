...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeHugeAmount"))
        settings.setValue("nAnonymizeHugeAmount", 1000);
    nAnonymizeHugeAmount = settings.value("nAnonymizeHugeAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeHugeAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeHugeAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeHugeAmount:
            return QVariant(nAnonymizeHugeAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeHugeAmount:
            nAnonymizeHugeAmount = value.toInt();
            settings.setValue("nAnonymizeHugeAmount", nAnonymizeHugeAmount);
            emit AnonymizeHugeAmountChanged(nAnonymizeHugeAmount);
            break;
        default:
            break;
        }
