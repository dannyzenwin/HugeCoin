...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeHugeAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeHugeAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeHugeAmount = strAnonymizeHugeAmount.remove(strAnonymizeHugeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHugeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeHugeAmount*COIN) nMaxToAnonymize = nAnonymizeHugeAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeHugeAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeHugeAmount));
        strAnonymizeHugeAmount = strAnonymizeHugeAmount.remove(strAnonymizeHugeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHugeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
